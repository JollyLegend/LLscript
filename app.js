...
const reloadButton = document.querySelector("#reload-script");
...
reloadButton.addEventListener("click", loadScript);

localFile.addEventListener("change", async (event) => {
  const [file] = event.target.files;
  if (!file) return;
  renderScript(await file.text());
  closeControls();
});
